...
def test_clock_reads_persisted_dates_and_shows_both_labels():
    ...
    tmpl = env.from_string(env.loader.get_source(env, 'tournament/cards/c1_header_nav.html')[0].split('          {% if not tournament.needs_availability_gate %}')[0])
...
def test_missing_end_is_not_silently_invented():
    ...
    template = (ROOT / 'templates/tournament/cards/c1_header_nav.html').read_text()
