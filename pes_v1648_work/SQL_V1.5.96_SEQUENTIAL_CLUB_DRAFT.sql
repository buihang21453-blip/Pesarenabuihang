-- V1.5.96: Run in Supabase SQL Editor before deploying the code.
...
begin
  ...
  if (select status from public.tournament_stages where tournament_id=p_tournament_id and stage_code='stage1') is distinct from 'completed'
     ...
     or exists (select 1 from public.tournament_matches where tournament_id=p_tournament_id and stage_code in ('league','knockout')) then
     raise exception 'Chỉ random khi GĐ1 hoàn tất, GĐ2 chưa có lịch/trận';
  end if;
