-- V1.5.98: deploy DB patch once. Replace both RPC definitions without calling them.
-- Fix: count ONLY active participants, preserve inactive membership rows, tickets and reward history.
-- Run BEFORE deploying V1.5.98 source. No club allocations are changed by this migration.
...
begin
  ...
  if (select count(*) from public.tournament_members where tournament_id=p_tournament_id and status='active') <> 16 then
    raise exception 'Giải phải có đúng 16 HLV đang tham gia';
  end if;
  select setting_value into v_state from public.tournament_settings
    where tournament_id=p_tournament_id and setting_key='club_draft_v2' for update;
  ...
  for v_row in select value from jsonb_array_elements(p_assignments) loop
    ...
    select pot_no into v_pot from public.tournament_members
      where tournament_id=p_tournament_id and user_id=v_uid and status='active' for update;
