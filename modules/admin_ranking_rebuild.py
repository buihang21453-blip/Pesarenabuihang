    ...
    for match in sorted(effective_matches, key=_sort_key):
        ...
        if repeat_opponent_rules_enabled:
            ...
            if encounter_number >= 7:
                ...
                affect_streak = True
                # Giữ nguyên delta hòa do RP Engine sinh ra bằng seed theo match_id.
                # Nhờ vậy rebuild cho ra đúng cùng một kết quả random 1..6.
                rp1 = _int(player1.get("rank_points"))
                rp2 = _int(player2.get("rank_points"))
                repeat_details["draw_bonus_applied"] = bool(
