-- PES Arena V1.6.20 — Replace V1.6.18 function BEFORE deploying V1.6.20 source.
...
BEGIN
  ...
  IF EXISTS(SELECT 1 FROM public.tournament_stages
    WHERE tournament_id=p_tournament_id AND stage_code='league' AND status='open') THEN
    RAISE EXCEPTION 'GĐ2 đã mở thi đấu; không thể dùng vé thưởng sớm';
  END IF;
